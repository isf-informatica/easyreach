...
export default function Dashboard() {
  ...
  // status could be string or object
  const getStatus = (school) => {
    if (typeof school.status === 'string') return school.status
    return 'pending'
  }

  const statusColor = (status) => {
    if (status === 'active') return 'bg-green-50 text-green-600'
    if (status === 'killed') return 'bg-red-50 text-red-600'
        ...
        <div className="grid grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            ...
            <div className="text-2xl font-semibold text-green-600">
              {schools.filter(s => getStatus(s) === 'active').length}
            </div>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="text-xs text-gray-500 mb-1">Pending</div>
            <div className="text-2xl font-semibold text-amber-600">
              {schools.filter(s => getStatus(s) === 'pending').length}
            </div>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="text-xs text-gray-500 mb-1">Killed</div>
            <div className="text-2xl font-semibold text-red-600">
              {schools.filter(s => getStatus(s) === 'killed').length}
            </div>
          </div>
        </div>
